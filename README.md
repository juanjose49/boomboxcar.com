...
The booking summary includes a collapsed coupon-code field. Coupon definitions stay private in `data/coupons.json` and are created, edited, activated, or deactivated through the authenticated admin page. Coupons may be percentage discounts or fixed US-dollar amounts. For payment-flow testing, a `TEST_SALE` coupon configured at 100 percent dynamically discounts any booking to a one-cent balance instead of zero. Do not use a 100 percent code as a public promotion. Codes are normalized to uppercase and may contain letters, digits, underscores, and hyphens. The backend loads the current coupon record, verifies that it is active, recalculates the selected package and modifiers, and adds the discount to the Square order. A fixed coupon that equals or exceeds the current subtotal is rejected before booking creation because the payment route requires at least $0.01 due. Do not configure the same promotion again as a Square Marketing coupon because that could create two independently redeemable versions of the discount.
...
Dashboard changes are saved atomically to `data/partners.json` and `data/coupons.json` with owner-only permissions. These private files survive normal deployments because the deploy script does not remove `data`. Before the first partner change, `BOOMBOXCAR_PARTNERS` remains the partner seed configuration. Coupons have no environment-variable fallback and must be managed through the dashboard.

The command-line generator remains available as a fallback or for creating an initial environment seed:
