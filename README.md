...
Google Analytics records the recommended ecommerce funnel events `select_item`, `begin_checkout`, `add_payment_info`, and `purchase`, along with payment errors and confirmation actions. The `purchase` event is emitted only after the confirmation API verifies completed payment and uses the final Square amount plus the unique Square order ID as its transaction ID. The confirmation page disables the automatic page view and sends a sanitized `/confirmation/` page location so its private confirmation token is never included in Analytics.
