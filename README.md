...
The booking summary includes a collapsed coupon-code field. Coupon definitions stay private on the server in `BOOMBOXCAR_COUPONS`, using comma-separated `CODE:TYPE:VALUE` entries. `TYPE` is `PERCENT` or `FIXED`, where fixed values are US dollars. For example, `WELCOME10:PERCENT:10,BOOM50:FIXED:50` creates a 10 percent code and a $50 code. Codes are normalized to uppercase and may contain letters, digits, underscores, and hyphens. The backend validates the code, selected package, and modifiers, calculates the discount in cents, and adds the discount to the Square order for both Apple Pay and hosted checkout. Do not configure the same promotion again as a Square Marketing coupon because that could create two independently redeemable versions of the discount.
