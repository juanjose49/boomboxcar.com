...
rm -f "$app_root/server.js" "$app_root/package.json"
cp -R "$repo_root/dist/app/server" "$app_root/server"
cp "$repo_root/dist/app/server.js" "$repo_root/dist/app/package.json" "$app_root/"

rm -rf "$public_root/es" "$public_root/confirmation" "$public_root/images"
