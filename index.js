...
function syncExperienceOrder() {
  ...
  if (!main) return;
  const experienceOrder = ['.experience-video', '.campaign-offer', '.value-proposition', '.gallery-party'];
  const remainingOrder = ['.partner-pass', '.booking', '.features', '.pricing', '.events', '.faqs', '.contact'];
...
function bindGalleryThemeTransition() {
  const gallery = document.querySelector('[data-experience="party-images"]');
  const grid = gallery?.querySelector('.grid');
  if (!grid) return;
  const dayFigures = Array.from(grid.querySelectorAll('.gallery-day-image'));
